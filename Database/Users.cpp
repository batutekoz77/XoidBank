...
std::optional<std::string> UsersBase::getFullNameById(int userId) {
    std::string fullName;
    bool found = false;

    try {
        db << "SELECT FullName FROM Users WHERE UserId = ?;"
            << userId
            >> [&](std::string name) { fullName = name; found = true; };
    }
    catch (const std::exception& e) {
        std::cerr << "getFullNameById error: " << e.what() << std::endl;
        return std::nullopt;
    }
