...
XoidBank is a secure banking web application built from scratch in C++. It handles authentication, mandatory two-factor verification, tiered session management, Dutch banking compliance (BSN validation), and a live dashboard backed by a real accounts/transactions layer — all served through a self-contained C++ backend with a per-page HTML/CSS/JS frontend.

> ⚠️ **Status:** Active development. Authentication, registration, 2FA, and the Dashboard data feed are fully functional. Dashboard action buttons (transfers, settings, etc.) are UI-complete but not yet wired to the backend.
...
- **Live Dashboard data** — real account balance, IBAN, and transaction history pulled server-side from the authenticated session, never trusting client-supplied identity
...
<p align="center">
  <img src="Showcase/signup.png" width="45%" />
  <img src="Showcase/signin.png" width="45%" />
</p>
<p align="center">
  <img src="Showcase/2fa.png" width="45%" />
  <img src="Showcase/dashboard.png" width="45%" />
</p>
...
### Dashboard
- [x] Dashboard UI (frontend complete)
- [x] `GET /Dashboard/Data` — session-authenticated backend endpoint (balance, IBAN, recent transactions)
- [x] `Accounts` / `Transactions` database schema (integer-based balances, no floating-point money)
- [ ] **Settings** button
- [ ] **Transfer** button
- [ ] **Payment request** button
- [ ] **Scan QR** button
- [ ] **Recent transactions** (view full history / click-through)

### Later
- [ ] Audit logging (login attempts, transfers, password changes)
- [ ] Account settings backend (password change, phone/email updates)
...
│   ├── Dashboard/        # Dashboard.hpp/.cpp — session-authenticated account/transaction data
...
│   └── Accounts.hpp/.cpp # Accounts + Transactions tables, balance handling (integer cents)
├── Session/
...
- Account balances and transaction amounts are stored as integers (minor currency units), never floats
- `/Dashboard/Data` identifies the user exclusively from the server-side session — no client-supplied ID, email, or balance is ever trusted
